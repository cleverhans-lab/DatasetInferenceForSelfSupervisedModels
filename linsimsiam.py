    ...
    if args.save == "True":
        if args.modeltype == "stolen":
            torch.save(model.state_dict(),
                       f"{pathpre}/SimCLR/downstream/stolen/imagenetenc/{args.datasetsteal}_down_{args.dataset}.pth.tar")
        elif args.modeltype == "victim":
            torch.save(model.state_dict(),
                       f"{pathpre}/SimCLR/downstream/victim/imagenet_down_{args.dataset}_r50.pth.tar")
...
def validate(val_loader, model, criterion, epoch, args):
    ...
    with torch.no_grad():
        ...
        logging.debug(
            ' Epoch: {epoch} * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f}'
            .format(epoch=epoch, top1=top1, top5=top5))
